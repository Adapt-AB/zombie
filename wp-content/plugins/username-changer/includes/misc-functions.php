...
function username_changer_process( $old_username, $new_username ) {
	...
	if ( $user_id ) {
		...
		$new_nicename = str_replace(".", "-", $new_username);
		...
		if ( false !== $wpdb->query( $q ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery
			...
			$qnn = $wpdb->prepare( "UPDATE $wpdb->users SET user_nicename = %s WHERE user_login = %s", $new_nicename, $new_username ); // phpcs:ignore WordPressVIPMinimum.Variables.RestrictedVariables
